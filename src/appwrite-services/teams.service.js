...
class AppWriteTeams {
    ...
    async createMember(TEAM_ID, EMAIL, ROLES_ARRAY) {
        try{
            const response = await this.team.createMembership(TEAM_ID, EMAIL, ROLES_ARRAY);
    ...
    async upadateRoles(TEAM_ID, MEMBER_ID, UPDATED_ROLES_ARRAY) {
        try{
            const response = await this.team.updateMembershipRoles(TEAM_ID, MEMBER_ID, UPDATED_ROLES_ARRAY);
            console.log(response)
            return response;
        } catch (error) {
            console.log("ERROR in getAllDocs():: ", error.response)
